...
const blocks = [
  {
    height: 891042, size: "14.2 kB", votes: 5, tkt: 3, rev: 1, age: "2m 14s",
    rows: [
      { type: "VAR", txn: 4, amt: "6.2501", size: "1.1 kB", subs: [] },
      { type: "SKA", txn: 3, amt: "6.6343", size: "3.8 kB", subs: [
  ...
  {
    height: 891041, size: "9.6 kB", votes: 5, tkt: 1, rev: 0, age: "4m 07s",
    rows: [
      { type: "VAR", txn: 2, amt: "4.0000", size: "890 B", subs: [] },
      { type: "SKA", txn: 2, amt: "6.5021", size: "2.1 kB", subs: [
  ...
  {
    height: 891040, size: "6.3 kB", votes: 4, tkt: 2, rev: 2, age: "6m 51s",
    rows: [
      { type: "VAR", txn: 3, amt: "3.9100", size: "750 B", subs: [] },
      { type: "SKA", txn: 7, amt: "4.2400", size: "1.7 kB", subs: [
  ...
  {
    height: 891039, size: "18.1 kB", votes: 5, tkt: 4, rev: 0, age: "9m 33s",
    rows: [
      { type: "VAR", txn: 6, amt: "10.3100", size: "3.3 kB", subs: [] },
      { type: "SKA", txn: 9, amt: "5.0110", size: "4.2 kB", subs: [
  ...
  {
    height: 891038, size: "11.4 kB", votes: 3, tkt: 2, rev: 1, age: "12m 05s",
    rows: [
      { type: "VAR", txn: 5, amt: "7.6600", size: "2.0 kB", subs: [] },
      { type: "SKA", txn: 4, amt: "3.7800", size: "2.5 kB", subs: [
  ...
  blocks.forEach((block, bi) => {
    ...
    if (bOpen) {
      block.rows.forEach((row, ri) => {
        const rKey  = bKey + "r" + ri;
        const rOpen = !!state[rKey];
          ...
          <tr class="row-txn" ${hasSub ? `data-key="${rKey}"` : ""}>
            ...
            <td class="r">
              <div class="cell num i1" style="justify-content:flex-end;gap:4px">
                ${hasSub
                  ? `<span class="caret${rOpen ? " open" : ""}" data-key="${rKey}">&#9658;</span>`
...
function updateCount() {
  const n = document.querySelectorAll("tr.row-root, tr.row-txn, tr.row-sub").length;
...
document.getElementById("btn-expand").addEventListener("click", () => {
  blocks.forEach((block, bi) => {
    state["b" + bi] = true;
    block.rows.forEach((row, ri) => {
      if (row.type === "SKA" && row.subs.length) state["b" + bi + "r" + ri] = true;
    });
  });
  render();
});
